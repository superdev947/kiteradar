...
const myInvites = ({navigation}) => {
  ...
  const loadList = () => {
    setLoading(true);
    api
      .get(`invites?user=${user.id}&status=0`)
      .then((resp) => {
        console.log('invites ativos', resp.data);
        setList(resp.data);
        setLoading(false);
        setRefresh(false);
      })
  ...
  const encerrarInvite = (item) => {
    api
      .put(`invites/${item.id}`, {status: 4})
      .then((resp) => {
        ...
        <FlatList
          ...
          renderItem={({item}) => (
            <CardInvite
              ...
              edit
              onPressEdit={() => navigation.navigate('EditInvites', {item})}
              encerrar
              onPressEncerrar={() => encerrarInvite(item)}
            />
          )}
