...
const Edit = ({navigation, route}) => {
  ...
  const carregarDados = () => {
    setFotos([{add: true}, ...item.AdPhotos]);
    dispatch(setCity(item.city));
    setData({
      ...data,
      price: maskMoney(item.price),
      title: item.title,
      ...
      .then((results) => {
        ...
        axios
          ...
          .then((results) => {
            ...
            setLoading(false);
            Alert.alert(displayName, 'Anúncio editado com sucesso');
            navigation.goBack();
          })
          .catch((error) => {
            setLoading(false);
  ...
  const deletePhotos = () => {
    ...
    item.AdPhotos.forEach((item) => {
      if (!listaFotos.includes(item)) {
        listFotosDeletadas.push(api.delete(`adv/photo/${item.id}`));
      }
    });
        ...
        <FlatList
          ...
          renderItem={({item, index}) => {
            if (item.add) {
              ...
              return (
                <ItemImage
                  source={item.id ? item.photo.url : item.uri}
                  onPress={() => {
                    setFoto(item);
