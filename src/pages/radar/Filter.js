...
import {
  ...
  setRadarStateList,
  setRadarCityList,
} from '../../store/modules/radar/actions';
...
const Filter = ({navigation}) => {
  ...
  const loadState = () => {
    api
      .get('address/state')
      .then((resp) => {
        dispatch(setRadarStateList(resp.data))
      })
      .catch((error) => {
  ...
  const loadCity = () => {
    api
      .get('address/city')
      .then((resp) => {
        dispatch(setRadarCityList(resp.data))
      })
      .catch((error) => {
        console.log('error', error);
      });
  ...
  const filtrar = () => {
    dispatch(setFilterRadar({...filter}));
    navigation.goBack();
  };
    ...
    <ScrollView contentContainerStyle={styles.container}>
      <KeyboardAvoidingView
        ...
        <Button
          title="Limpar filtro"
          icon="trash-2"
          onPress={() => dispatch(setFilterRadar(defaultValues))}
